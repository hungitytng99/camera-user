...
const contactSchema = Yup.object().shape({
  ...
  phone: Yup.string()
    .matches(phoneReg, 'Vui lòng nhập 1 số điện thoại đúng')
    .required('Không để trống trường này'),
  email: Yup.string()
    .email('Vui lòng nhập địa chỉ email đúng')
    .required('Không để trống trường này'),
});
        ...
        {({ errors, touched }) => (
          <Form>
            ...
            <div className="contact-form__btn-group">
              <div className="contact-form__message-submit">
                {messageAfterValidate}
              </div>
              <button className="contact-form__btn send" type="submit">
                Gửi đi
              </button>
            </div>
          </Form>
