...
const Category = (props) => {
  ...
  const [detailCategoryState, setDetailCategoryState] =
    useState(detailCategory);
  const [nextPage, setNextPage] = useState(4);
  const [hasMoreProductState, setHasMoreProductState] =
    useState(hasMoreProduct);
  ...
  const handleFilterPrice = async (e) => {
    ...
    const filterProduct = await productService.listProductByCategorySlug(
      detailCategoryState.slug,
      { productsPerPage: 18, pageNumber: 1, ...filterOption[filterValue] },
    );
    setDetailCategoryState({
      ...detailCategoryState,
      listProduct: filterProduct.data,
    });
  };

  const getMoreProduct = async (e) => {
    console.log('CURRENT FILTER: ', currentFilter);
    const listProduct = await productService.listProductByCategorySlug(
      detailCategoryState.slug,
      {
        productsPerPage: 6,
        pageNumber: e.target.dataset.nextpage,
        ...currentFilter,
      },
    );
    setDetailCategoryState({
      ...detailCategoryState,
      listProduct: [...detailCategoryState.listProduct, ...listProduct.data],
    });
  ...
  useEffect(() => {
    ...
    const checkShowMore = async () => {
      const moreProduct = await productService.listProductByCategorySlug(
        detailCategoryState.slug,
      ...
      <Layout>
        ...
        <CardWithTitle title={detailCategoryState.name}>
          <Row>
            <Col md={3} className="hide-on-768">
              <div className="category__filter">
                  ...
                  <ul className="category__filter-category-list">
                    {listCategory.map((category) => {
                      return (
                        <li
                          key={category.id}
                          className="category__filter-category-item"
                        >
                          <a
                ...
                <div className="category__filter-price">
                  <div className="category__filter-title">Lọc theo giá</div>
                  <ul
                    className="category__filter-price-list"
            ...
            <Col xs={12} md={9}>
              <Row>
                {detailCategoryState.listProduct.length === 0 && (
...
export async function getServerSideProps(context) {
  ...
  const listProduct = await productService.listProductByCategoryId(
    ...
    { productsPerPage: 18, pageNumber: 1 },
  );
  detailCategory.data = {
    ...detailCategory.data,
  ...
  const moreProduct = await productService.listProductByCategoryId(
    detailCategory.data.id,
    { productsPerPage: 6, pageNumber: 4 },
  );
  if (moreProduct.data.length > 0) {
    hasMoreProduct = true;
  }
