...
const News = (props) => {
  ...
  const getMorePost = async (e) => {
    ...
    const morePost = await postService.listPost({
      postsPerPage: 6,
      pageNumber: nextPage,
    });
    ...
    <>
      ...
      <Layout>
        <Row className="news__header">
          <h3 className="news__header-title">Tin tức</h3>
          <div className="news__decorate">
            <div className="news__decorate-img-box">
              <Image
                className="news__decorate-img"
                src={ImagesPath.NEWS_DECORATE}
                alt="trang tri tin tuc"
                layout="fill"
              />
            </div>
          </div>
        ...
        <Row>
              ...
              <Col key={post.id} xs={12} sm={6} md={4}>
                <Link href={post.slug} passHref prefetch={false}>
                  <a className="news__link">
                    ...
                    <div className="news__demo-content text_over_flow_2">
          ...
          {hasMoreState ? (
            <div className="product__has-more-box">
              <div
                className="product__has-more"
...
export async function getServerSideProps(context) {
  ...
  const nextPost = await postService.listPost({
    postsPerPage: 6,
    pageNumber: 4,
  });
  let hasMore = false;
